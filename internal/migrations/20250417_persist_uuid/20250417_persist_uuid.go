...
// 20250417_persist_uuid was written against the v2 single-app bucket layout
// ({prefix}/{branch}/{runtimeVersion}/{updateId}). Its purpose was to backfill
// update.json UUIDs on pre-existing v2 data.
//
// In v3 the migration is a no-op: v3 writes update.json at publish time, so
// any data created by a v3 server already satisfies the invariant this
// migration enforced. The only remaining path that can carry un-UUID-ed data
// is a v2→v3 in-place upgrade, and that path is handled by
// 20260422_v3_scope_data_under_appid — which re-paths v2 data into the new
// {appId}-scoped layout and preserves whatever UUIDs the v2 server had
// already written. Running the old iteration against the v3 layout would
