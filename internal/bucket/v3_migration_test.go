...
func newV2LayoutBucket(t *testing.T) *LocalBucket {
	t.Helper()
	base := t.TempDir()
...
func TestLocalBucket_MoveRootEntriesUnder_SkipsNonBranchShapedEntries(t *testing.T) {
	...
	// shape isn't a v2 branch — leave it alone.
	base := t.TempDir()
	// A legit v2 branch — should move.
	writeFile(t, filepath.Join(base, "branch-a", "1", "12345", ".check"))
	// A v3-looking appId dir (depth-4 .check) — must be left alone.
	writeFile(t, filepath.Join(base, "other-app-uuid", "branch-a", "1", "67890", ".check"))
	// A random directory with no v2 shape at all — must be left alone.
...
func TestLocalBucket_MoveRootEntriesUnder_RejectsAppIdCollidingWithV1Branch(t *testing.T) {
	// An opera tor upgrades to v3 and picks EXPO_APP_ID="staging" — but
	// their v2 bucket already has a branch literally called "staging".
	// Auto-moving everything under staging/ would nest the v2 "staging"
	// branch as staging/staging/... — data loss in place. Require the
	// operator to resolve manually.
	base := t.TempDir()
	writeFile(t, filepath.Join(base, "staging", "1", "12345", ".check")) // v2 branch named "staging"
	...
	require.Error(t, err)
	assert.ErrorIs(t, err, ErrAppIdCollidesWithV2Branch)
...
func TestLocalBucket_MoveRootEntriesUnder_AcceptsV2AppIdDirectoryWithSameName(t *testing.T) {
	// The {appId}/ directory pre-exists BUT it is v3-shaped (marker at
	// depth 3 inside it, i.e. {appId}/{branch}/{rv}/{update}/.check). The
	// migration must NOT confuse it for a v2-branch collision — that's
	// just "already migrated" and the run should complete without error.
	base := t.TempDir()
	writeFile(t, filepath.Join(base, "app-1", "branch-a", "1", "12345", ".check")) // v3 shape
	writeFile(t, filepath.Join(base, "branch-b", "1", "67890", ".check"))          // v2 branch
	writeFile(t, filepath.Join(base, ".migrationhistory"))
	b := &LocalBucket{BasePath: base}

	require.NoError(t, b.MoveRootEntriesUnder("app-1"))

	// The existing v3 content stays put, and branch-b gets moved in.
	assert.FileExists(t, filepath.Join(base, "app-1", "branch-a", "1", "12345", ".check"))
	assert.FileExists(t, filepath.Join(base, "app-1", "branch-b", "1", "67890", ".check"))
...
func TestLocalBucket_MoveRootEntriesUnder_NoOpOnFullyV2Bucket(t *testing.T) {
	// If every non-ledger top-level entry is already v3-shaped, the
...
func TestV2BranchTripleFromMarker(t *testing.T) {
	cases := map[string]struct {
		triple string
		ok     bool
	}{
		// Confirmed v2 markers at exactly segment 4.
		...
		// v3 key from another app — marker at segment 5. Must NOT confirm
		...
		// v2 nested assets are not markers themselves — their triple is
	...
	for k, want := range cases {
		t.Run(k, func(t *testing.T) {
			got, ok := v2BranchTripleFromMarker(k)
...
func TestInConfirmedTriple(t *testing.T) {
	...
	// v2 own-branch objects (flat and nested) — both belong to the triple.
	...
	// v3 keys from another app — first 3 segments do not match any
	// confirmed triple.
