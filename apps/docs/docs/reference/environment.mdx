...
- **Single-app** — flat env vars (`EXPO_APP_ID`, `EXPO_ACCESS_TOKEN`, `KEYS_STORAGE_TYPE`, …). Unchanged from v2.
...
**Path A — single app (keeps v2 env var names unchanged)** — set:
...
:::caution v3 is a breaking upgrade
The **env var names are identical to v2** on the single-app path, but the
server still requires **client-side changes** (every app must now send an
`expo-app-id` header — see the [Migration guide](/docs/advanced/migration-v3-to-v3))
**and** the underlying bucket layout changed from `{prefix}/{branch}/…` to
`{prefix}/{appId}/{branch}/…`. Existing v2 bucket data is not read by a v3
...
Since v3, Expo signing keys are part of each app's `keys` block in
...
| Name | Required | Description | Example | Reference |
| --- | --- | --- | --- | --- |
| `SKIP_V2_TO_V3_BUCKET_MIGRATION` | ❌ | Set to `true` to opt out of the automatic one-shot bucket re-path that moves v2 data under `{appId}/` on first v3 boot. Useful when you want to re-path the data manually. Only meaningful on the single-app flat-env path. | `true` | [Ref](/docs/advanced/migration-v3-to-v3) |
