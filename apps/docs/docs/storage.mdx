...
**Expo Open OTA** supports storage solutions for hosting your update assets: **Amazon S3**, **Google Cloud Storage (GCS)** and **Local File System**. This guide will help you set up your storage solution and configure your server to use it.
...
<Tabs queryString="storage" defaultValue="s3">
  ...
  <TabItem value="gcs" label="Google Cloud Storage">
    ...
    # For signed URL generation (uploads and optional asset redirects):
    GOOGLE_APPLICATION_CREDENTIALS_B64=<Base64-encoded-service-account-json>
    ```

    The service account must have least-privilege permissions for your bucket. For uploads and deletes, grant storage.objectCreator and storage.objectAdmin accordingly. Signed URLs are generated using the service account private key in the JSON.

    You don't need to make the objects public. The server will generate signed URLs for uploading files. If CDN redirection is enabled for GCS (direct signed URLs), assets are served by GCS via short-lived signed URLs.
  </TabItem>
