...
Upgrading from v2? Check the [Migration guide](/docs/advanced/migration-v3-to-v3) — there are **required client-side changes** (native rebuild) even when the server config stays identical.
:::
...
:::info
This ID is set as the `EXPO_APP_ID` environment variable on the server, and the client must send it as the `expo-app-id` HTTP header on every manifest and asset request (configured in `app.config.ts` → `updates.requestHeaders`; changing this value requires a **native rebuild** — see the [Migration guide](/docs/advanced/migration-v3-to-v3) for the full procedure).
:::
