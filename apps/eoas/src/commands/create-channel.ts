...
export default class CreateChannel extends Command {
  ...
  static override description = 'Create a new channel and map it to a branch';
  ...
  public async run(): Promise<void> {
    const { flags } = await this.parse(CreateChannel);
    ...
    if (!response.ok) {
      Log.error(`Failed to create channel: ${await response.text()}`);
      process.exit(1);
    }

    Log.withInfo(`Channel "${channel}" created and mapped to branch "${branch}"`);
  }
}
