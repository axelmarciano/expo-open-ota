...
export async function resolveServerUrl({
  config,
  customServerUrl,
}: ResolveServerUrlOptions): Promise<string> {
  const updateUrl = customServerUrl ?? config.updates?.url;

  if (!updateUrl) {
    throw new Error('Could not resolve server URL from Expo config or flags.');
  }
